...
const getProjectStageInfo = (currentAnalysisStage: Project["analysisStage"]): ProjectStageInfo => {
	...
	const projectStatus: ProjectStageInfo = {
		...
		initialInfoStatus: currentStageNum > 0 ? "completed" : currentStageNum === 0 ? "in-progress" : "not-started",
		technicalStatus: currentStageNum > 1 ? "completed" : currentStageNum === 1 ? "in-progress" : "not-started",
		ghgStatus: currentStageNum > 2 ? "completed" : currentStageNum === 2 ? "in-progress" : "not-started",
		finalStatus: currentStageNum > 3 ? "completed" : currentStageNum === 3 ? "in-progress" : "not-started",
		risksStatus: currentStageNum > 4 ? "completed" : currentStageNum === 4 ? "in-progress" : "not-started",
	}
...
const ProjectView: React.FC<ProjectViewProps> = ({ project, onBack }) => {
  ...
  const completeProjectStage = () => {
	if (projectStage === "Complete, and Excluded") {
		...
		setProjectStage((Object.keys(projectStageOrder) as Project["analysisStage"][]).find(key => projectStageOrder[key] === currentStageNum + 1)!)
	}
  }
    ...
    <div className={styles.projectViewContainer}>
      ...
      <div className={styles.mainContent}>
        ...
        <div className={styles.rightPanel}>
          <ProjectUpdates updates={sampleUpdates.filter(update => update.projectId === project.id)} completeProjectStage={completeProjectStage} />
        </div>
      </div>
    </div>
  );
